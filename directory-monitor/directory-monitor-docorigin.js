...
class DirectoryMonitor {
	...
	static async _applyConfiguration(watcher) {
		...
		let mfData = fs.readFileSync(config.monitoredfoldersconfig2);
		...
		const newWatcher = Chokidar.watch(DirectoryMonitor._monitoredFolders, {
			...
			.on('ready', () =>
				log(logPrefix, 'directory-monitor-docorigin Initial folder scan complete. Started monitoring...')
			);
		...
		log(
			logPrefix,
			`Loaded Monitored Folders for Directory-Monitor-Docorigin|${JSON.stringify(
				DirectoryMonitor._monitoredFolders
			)}`,
			SUCCESS
		);
...
DirectoryMonitor.init(workOnce)
	.then(() => {
		log(logPrefix, 'directory-monitor-docorigin Initialized', SUCCESS);
	})
	.catch((err) => {
